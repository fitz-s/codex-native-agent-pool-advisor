...
7. For native-spawn claims, run `scripts/live-check.mjs --transcript <real_parent_transcript>` with expectation flags for the claim being made: `--expect-model`, `--expect-current-open`, and `--expect-all-closed`.
