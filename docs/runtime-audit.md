...
- `agent_type` is runtime transport, never model routing. Omit it unless the actual native tool requires it; roles/profiles must not supply model or effort.
- A failed native spawn is an error in the constructed call or runtime contract. It must not lead to type probes, fallback retries, missing-model retries, or another worker tool.
...
- During `PreToolUse(spawn_agent)`, advisor state is not consulted. Admission reads only the current-parent native edge/visible-child snapshot and current tool input; it does not acquire the advisor lock, scan transcripts, sanitize context, run maintenance, or mutate advisor/Codex state. It blocks only when native edge evidence is unavailable or shows a real shape/capacity conflict.
