...
- Model selection is mandatory for every non-fork `spawn_agent` call. Missing, blank, null, or non-string `model` values inherit or fail ambiguously and are treated as missing; native `agent_type` is not an exception. Terra/medium is the default reasoning-level choice, Luna/low is only for bounded locating, and Sol/high is for frontier parallel review, architecture, or high-risk work. This is hard-blocked only when Codex emits a supported `PreToolUse` event for the spawn path.
- `fork_context=true` is a runtime shape exception, not a model-routing strategy. If Codex rejects `fork_context=true` with `model`, the agent should remove `fork_context` and pass compact context for Luna/Terra/Sol routing, or omit `model` only when exact full-history fork is more important than model choice. That shape failure must not be treated as native-pool exhaustion.
...
- The hook does not route by `agent_type`. The enforceable model boundary is the explicit `model` field in every non-fork tool input; boolean `fork_context=true` is the only inheritance exception. Luna is the bounded locator lane for files/symbols/logs/DB rows/exact anchors, Terra is the default reasoning/light-executor lane, and Sol is the frontier lane for critic/code-reviewer/architecture/security/high-risk/final-approval work.
- Complex tasks may still use Luna only when Luna is used as a bounded anchor collector with exact slices, bounded search shape, output cap, and stop condition. Luna is not the right lane for persistent children, repeated follow-ups, broad unbounded grep, compiled/vendor trees, large log/DB dumps, multi-hop source+DB+log synthesis, or any task likely to trigger remote compaction. If a Luna lane compacts once, stop sending it follow-ups and escalate its anchors to Terra/parent.
