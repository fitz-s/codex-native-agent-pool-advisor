...
1. `node scripts/doctor.mjs` reports exactly one registration for `SessionStart`, `UserPromptSubmit`, `PreToolUse`, and `PostCompact`; `PostToolUse` and `PreCompact` are absent.
