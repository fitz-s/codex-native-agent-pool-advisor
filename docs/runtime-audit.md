...
- During `PreToolUse(spawn_agent)`, advisor write-lock contention alone is not a capacity failure. After a short lock wait, the hook performs a read-only current-parent native-edge snapshot; it allows a valid explicit route when that snapshot is authoritative and blocks only when the native edge query is unavailable or shows a real shape/capacity conflict. The fallback never mutates advisor or Codex state.
