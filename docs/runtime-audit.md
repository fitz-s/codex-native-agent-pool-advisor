...
- Live-check records same-response spawn batches, including nested `multi_tool_use.parallel` agent calls, but batches are valid when they fit the current budget. It fails when spawn outputs are missing, spawn/close outputs show runtime/tool failure, missing-model child creation occurs, or expectation mismatches.
