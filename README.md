...
- Advisor write-lock contention is not itself a capacity failure. For a supported spawn precheck, the hook short-waits then falls back to a read-only native-edge snapshot. It permits only a valid explicit route with authoritative free capacity and never writes advisor/Codex state on that path; an unreadable native edge query remains fail-closed.
